...
straight from the GitHub REST API and renders matrix-themed SVG cards that
live in this repo, so the README never depends on a third-party deployment.
"""
import json
import os
...
def build_status_card(user, repos, langs, badges):
    ...
    parts = [
        ...
        f'<rect x="1" y="1" width="{w-2}" height="{h-2}" rx="10" fill="{BLACK}" stroke="{GREEN}" stroke-width="2"/>',
        f'<text x="28" y="42" font-family="{FONT}" font-size="20" fill="{GREEN}">'
    ...
    for label, value in rows:
        ...
        parts.append(
            f'<text x="260" y="{y}" font-family="{FONT}" font-size="17" fill="{GREEN}">{esc(value)}</text>'
        )
        y += 30
    ...
    if badges:
        ...
        for row in badge_rows:
            ...
            for b, bw in row:
                parts.append(
                    f'<rect x="{x}" y="{y-22}" width="{bw}" height="28" rx="14" fill="{BLACK}" stroke="{GREEN}" stroke-width="1.5"/>'
                )
                parts.append(
                    f'<text x="{x + bw/2}" y="{y-3}" text-anchor="middle" font-family="{FONT}" font-size="13" fill="{GREEN}">{esc(b)}</text>'
                )
                x += bw + 14
            y += 40
...
def build_activity_card(events):
    ...
    parts = [
        f'<svg xmlns="http://www.w3.org/2000/svg" width="{w}" height="{h}" viewBox="0 0 {w} {h}">',
        f'<rect x="1" y="1" width="{w-2}" height="{h-2}" rx="10" fill="{BLACK}" stroke="{GREEN}" stroke-width="2"/>',
        f'<text x="28" y="38" font-family="{FONT}" font-size="20" fill="{GREEN}">'
    ...
    parts.append(
        f'<text x="{w/2}" y="{base_y+28}" text-anchor="middle" font-family="{FONT}" font-size="14" fill="{GREEN}">'
        f'{total} events</text>'
    )
    parts.append("</svg>")
    return "\n".join(parts)
